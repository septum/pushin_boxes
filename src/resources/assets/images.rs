...
impl Images {
    ...
    pub fn spawn_background(&self, commands: &mut Commands, component: impl Component) {
        let sprite = SpriteBundle {
            texture: self.background.clone(),
            ..default()
        };
        commands.spawn_bundle(sprite).insert(component);
    }

    pub fn from_map_entity(&self, entity: &MapEntity) -> Handle<Image> {
        match entity {
            MapEntity::W => self.entities.wall.clone(),
            MapEntity::F => self.entities.floor.clone(),
            MapEntity::Z => self.entities.zone.clone(),
            MapEntity::B | MapEntity::P => self.entities.pbox.clone(),
        }
    }
}
